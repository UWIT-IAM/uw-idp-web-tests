...
class TestNewSessionAndExistingSSOSession:
    ...
    def test_new_session_standard(self, secrets, netid):
        ...
        with self.utils.using_test_sp(sp):
            self.browser.get(f'{self.sp_url(sp)}/shibeval')
...
class TestCredentialsAndForcedReauth:
    ...
    """
    @pytest.fixture(autouse=True)
    def initialize(self, class_browser, utils, sp_url, secrets, netid, new_tab):
        self.browser = class_browser
        self.utils = utils
        self.sp_url = sp_url
        self.new_tab = new_tab
    ...
    def test_existing_sso_for_sign_in_with_forced_reauth(self, sp_domain):
        ...
        with self.utils.using_test_sp(sp):
            self.new_tab()
            self.browser.get(f'{self.sp_url(sp)}/shibevalforce')
            self.browser.send_inputs(self.netid, self.password)
            self.browser.click(Locators.submit_button)
            self.browser.wait_for_tag('h2', f'{sp_domain(sp)} sign-in success!')
...
def test_idp_initiated_sign_in(browser, netid, utils, sp_url, secrets, sp_domain):
    ...
    with utils.using_test_sp(sp):
        fresh_browser.get('https://idp-eval.u.washington.edu/idp/profile/SAML2/Unsolicited/SSO?providerId=https'
...
class TestNetIDCaseInsensitivityAndURLQueryParams:
    ...
    def test_case_insensitivity(self, netid, sp_domain):
        ...
        with self.utils.using_test_sp(self.sp):
            self.browser.get(f'{self.sp_url(self.sp)}/shibeval')
    ...
    def test_query_parameters(self,  netid):
        ...
        with self.utils.using_test_sp(self.sp):
            self.browser.snap()
            self.browser.get(f'{self.sp_url(self.sp)}/shibeval/q-param-test.aspx?fname=Joe&lname=Smith&age=30')
...
def test_blacklisted_cookie():
    """
    PWD-10	Blacklisted cookie (disuser case)
    Per Michael, this test is not yet appropriate for automation
    """
